...
export default function Dashboard({ user }: { user: UserPayload }) {
  ...
  const [vista, setVista] = useState<Vista>({ tipo: "procedimientos" });
  const [inwebExpandido, setInwebExpandido] = useState(false);
  ...
  useEffect(() => {
    if (vista.tipo === "procedimientos") loadProcedimientos();
  }, [loadProcedimientos, vista]);
  ...
  function irAProcedimientos(slug: string | null) {
    setSectorActivo(slug);
    setVista({ tipo: "procedimientos" });
  }

  function irAInstructivos(slug: string) {
    setVista({ tipo: "instructivos", slug });
    setInwebExpandido(true);
  }

  const sidebarW = 240;
  const esProcedimientos = vista.tipo === "procedimientos";
  const instructivoActivo = vista.tipo === "instructivos" ? vista.slug : null;

  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc" }}>
      <aside style={{
        width: sidebarW, flexShrink: 0, background: "white",
    ...
    <div style={{ fontSize: 14, color: "#9ca3af" }}>Procedimientos</div>
        ...
        <nav style={{ padding: "16px 12px", flex: 1, overflowY: "auto" }}>
          ...
          <button onClick={() => irAProcedimientos(null)} style={{ width: "100%", textAlign: "left", padding: "9px 10px", borderRadius: 8, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 500, background: esProcedimientos && sectorActivo === null ? "#eff6ff" : "transparent", color: esProcedimientos && sectorActivo === null ? "#2563eb" : "#374151", transition: "all 0.15s" }}>
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
            Todos los sectores
          </button>
  ...
  <>
    ...
    <button onClick={() => setVista({ tipo: "usuarios" })} style={{ width: "100%", textAlign: "left", padding: "9px 10px", borderRadius: 8, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 500, background: vista.tipo === "usuarios" ? "#eff6ff" : "transparent", color: vista.tipo === "usuarios" ? "#2563eb" : "#374151", transition: "all 0.15s" }}>
      ...
      Usuarios
          ...
          <button onClick={() => setInwebExpandido(!inwebExpandido)} style={{ width: "100%", textAlign: "left", padding: "9px 10px", borderRadius: 8, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 500, background: !esProcedimientos ? "#eff6ff" : "transparent", color: !esProcedimientos ? "#2563eb" : "#374151", transition: "all 0.15s" }}>
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
            Carpetas
            <svg style={{ marginLeft: "auto", transform: inwebExpandido ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M6 9l6 6 6-6"/></svg>
          </button>

          {inwebExpandido && (
            <div style={{ paddingLeft: 8 }}>
              {INSTRUCTIVOS.map(c => (
                <button key={c.slug} onClick={() => irAInstructivos(c.slug)} style={{ width: "100%", textAlign: "left", padding: "7px 10px", borderRadius: 8, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 500, background: instructivoActivo === c.slug ? "#eff6ff" : "transparent", color: instructivoActivo === c.slug ? "#2563eb" : "#374151", transition: "all 0.15s" }}>
                  <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
                  {c.nombre}
                  <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 600, background: instructivoActivo === c.slug ? "#dbeafe" : "#f3f4f6", color: instructivoActivo === c.slug ? "#2563eb" : "#9ca3af", borderRadius: 10, padding: "1px 6px" }}>
                    {c.archivos.length}
                  </span>
        ...
        <div style={{ padding: "16px 12px", borderTop: "1px solid #f3f4f6" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px" }}>
            <div style={{ width: 32, height: 32, borderRadius: 50, background: "linear-gradient(135deg, #dbeafe, #bfdbfe)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#2563eb", flexShrink: 0 }}>
            ...
            <button onClick={handleLogout} title="Cerrar sesión" style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af", padding: 4, borderRadius: 6 }}>
              <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>
            </button>
          </div>
        </div>
      </aside>
    ...
    <main style={{ flex: 1, padding: "32px 32px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <div style={{ position: "relative" }}>
                  <svg style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#9ca3af" }} width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                  <input type="text" placeholder="Buscar procedimientos..." value={buscar} onChange={e => setBuscar(e.target.value)} style={{ paddingLeft: 32, paddingRight: 14, paddingTop: 9, paddingBottom: 9, border: "1.5px solid #e5e7eb", borderRadius: 10, fontSize: 13, outline: "none", background: "white", width: 240, fontFamily: "inherit", color: "#374151" }} onFocus={e => e.target.style.borderColor = "#2563eb"} onBlur={e => e.target.style.borderColor = "#e5e7eb"} />
                </div>
                {canEdit && (
                  <button onClick={() => setShowModal(true)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", background: "linear-gradient(135deg, #2563eb, #1d4ed8)", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer", boxShadow: "0 2px 8px rgba(37,99,235,0.3)", fontFamily: "inherit" }}>
            ...
            {loading ? (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
                {[1,2,3,4,5,6].map(i => <div key={i} style={{ height: 160, borderRadius: 14, background: "#f3f4f6", animation: "pulse 1.5s infinite" }}/>)}
              </div>
              ...
              <div style={{ textAlign: "center", padding: "80px 0" }}>
                <div style={{ width: 64, height: 64, background: "#f3f4f6", borderRadius: 16, display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                  <svg width="28" height="28" fill="none" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
                {procedimientos.map((p, i) => (
                  <div key={p.id} onClick={() => setDetalle(p.id)} style={{ background: "white", borderRadius: 14, padding: "20px", border: "1.5px solid #f3f4f6", cursor: "pointer", transition: "all 0.2s", animation: `fadeIn 0.3s ease ${i * 0.05}s both` }}
                    onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "#dbeafe"; (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 16px rgba(37,99,235,0.08)"; (e.currentTarget as HTMLDivElement).style.transform = "translateY(-2px)"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "#f3f4f6"; (e.currentTarget as HTMLDivElement).style.boxShadow = "none"; (e.currentTarget as HTMLDivElement).style.transform = "none"; }}
                  >
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20, background: `${p.sector_color}15`, color: p.sector_color, letterSpacing: "0.02em" }}>{p.sector_nombre}</span>
                      <span style={{ fontSize: 11, color: "#9ca3af" }}>{new Date(p.updated_at).toLocaleDateString("es-AR", { day: "2-digit", month: "short" })}</span>
                    </div>
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", marginBottom: 8, lineHeight: 1.4 }}>{p.titulo}</h3>
                    {p.descripcion && <p style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", marginBottom: 16 }}>{p.descripcion}</p>}
                    <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: "auto", paddingTop: 12, borderTop: "1px solid #f9fafb" }}>
                      {p.total_archivos > 0 && <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "#6b7280" }}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>{p.total_archivos} archivo{p.total_archivos !== 1 ? "s" : ""}</span>}
                      {p.total_links > 0 && <span style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "#6b7280" }}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>{p.total_links} link{p.total_links !== 1 ? "s" : ""}</span>}
      ...
      {showModal && <ProcedimientoModal sectores={sectores} user={user} onClose={() => setShowModal(false)} onSave={() => { setShowModal(false); loadProcedimientos(); }} />}
      {detalle && <ProcedimientoDetalle id={detalle} user={user} onClose={() => setDetalle(null)} onUpdate={loadProcedimientos} />}
    </div>
  );
}
